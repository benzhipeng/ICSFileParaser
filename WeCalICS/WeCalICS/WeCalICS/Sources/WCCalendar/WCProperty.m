...
+ (instancetype) propertyFactory: (icalproperty *) p {
    ...
    switch ( icalproperty_isa(p)) {
        ...
        case ICAL_ATTENDEE_PROPERTY:
            property = [[WCPerson alloc] init];
            break;
        case ICAL_LOCATION_PROPERTY:
            property = [[WCLoc alloc] init];
            break;
        case ICAL_GEO_PROPERTY:
            property = [[WCGeo alloc] init];
            break;
        default:
            property = [[WCProperty alloc] init];
            break;
    }
    return property;
}
...
+(instancetype) propertyWithIcalProperty: (icalproperty *) p {
    ...
    WCProperty * property = [WCProperty propertyFactory:p];
    ...
    if (property) {
        ...
        switch (property.valueKind) {
            ...
            case ICAL_NO_VALUE:
                ...
                break;
            case ICAL_GEO_VALUE:
                property.value = [property geoFromValue:v];
                break;
        ...
        if(property.kind == ICAL_X_PROPERTY){
            property.propertyName = [[NSString alloc] initWithCString:icalproperty_get_x_name(p) encoding:NSUTF8StringEncoding];
        }else {
            property.propertyName = [[NSString alloc] initWithCString:icalproperty_get_property_name(p) encoding:NSUTF8StringEncoding];
        }
...
- (NSDictionary*)geoFromValue:(icalvalue *) v {
    
    NSString* geoValue = [NSString stringWithCString: icalvalue_as_ical_string(v) encoding: NSUTF8StringEncoding];
    NSArray* tmp = [geoValue componentsSeparatedByString:@";"];
    return @{@"lat":[tmp firstObject],@"lon":[tmp lastObject]};
}
...
- (struct icalgeotype) icalgeotypeFromObject:(id)geo{
    struct icalgeotype ical_geo;
    if(!geo){
        ical_geo.lon = 0.0;
        ical_geo.lat = 0.0;
    }else {
        ical_geo.lat = [geo[@"lat"] floatValue];
        ical_geo.lon = [geo[@"lon"] floatValue];
    }
    return ical_geo;
}
...
-(icalvalue *) icalBuildValue {
    ...
    switch (self.valueKind) {
        ...
        case ICAL_REQUESTSTATUS_VALUE:
            ...
            break;
        case ICAL_GEO_VALUE:
            value = icalvalue_new_geo([self icalgeotypeFromObject:self.value]);
            break;
...
-(icalproperty *) icalBuildProperty{
    ...
    icalproperty * ical_property = NULL;
    if  (self.kind == ICAL_X_PROPERTY){
        ical_property = icalproperty_new (ICAL_X_PROPERTY);
        icalproperty_set_x_name (ical_property, [self.propertyName UTF8String]);
    }else {
        ical_property = icalproperty_new(self.kind);
    }    
...
- (NSString*)stringForRecurrence{
    icalvalue* value = icalvalue_new_recur([self icalrecurrencetypeFromObject:self.value]);
    char *ruleC = (char *)icalvalue_as_ical_string(value);
    NSString* rrlueString = @"";
    if (ruleC != NULL) {
        rrlueString = [NSString stringWithCString:ruleC encoding:NSUTF8StringEncoding];
    }
    return rrlueString;
}
...
- (instancetype)copyWithZone:(NSZone *)zone {
    WCProperty *object = [[[self class] allocWithZone:zone] init];
...
-(BOOL) isEqual: (WCProperty *) property {
